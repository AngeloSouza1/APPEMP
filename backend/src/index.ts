...
let cloudinaryMonitorTimer: NodeJS.Timeout | null = null;
...
const normalizarNfStatus = (valor: unknown): "PENDENTE" | "ANTECIPADA" => {
...
const normalizarExpoPushToken = (valor: unknown): string | null => {
...
const desativarTokensInvalidos = async (tokens: string[]) => {
  ...
  try {
    ...
    for (const bloco of blocos) {
      ...
      json.data.forEach((item, index) => {
        const erro = item?.details?.error;
        if (item?.status === "error" && erro === "DeviceNotRegistered") {
...
const cloudinaryMonitoramentoHabilitado = () =>
  Boolean(
...
const buscarUsoCloudinary = async () => {
  ...
  const usageBytes = Number(payload.storage?.usage || 0);
  const limitBytes = Number(payload.storage?.limit || CLOUDINARY_STORAGE_LIMIT_BYTES);
...
const enviarAlertaCloudinaryPorEmail = async (dados: {
  ...
  const text = [
    ...
    backup?.batchKey ? `Lote: ${backup.batchKey}` : null,
    typeof backup?.filesBackedUp === "number" ? `Arquivos salvos: ${backup.filesBackedUp}` : null,
    typeof backup?.filesFailed === "number" ? `Falhas: ${backup.filesFailed}` : null,
    typeof backup?.brokenUrlsCleaned === "number" ? `URLs quebradas removidas do banco: ${backup.brokenUrlsCleaned}` : null,
    backup?.totalBytesFormatted ? `Tamanho total: ${backup.totalBytesFormatted}` : null,
    backup?.downloadUrl ? `Download do backup: ${backup.downloadUrl}` : null,
  ]
    .filter(Boolean)
    .join("\n");
...
const baixarArquivoCloudinaryParaBackup = async (params: {
  ...
  if (!response.ok) {
    const error = new Error(`Falha ao baixar arquivo (${response.status})`) as Error & { status?: number };
...
const compactarLoteBackupCloudinary = async (backupRoot: string, batchKey: string) => {
...
const montarDownloadPathBackupCloudinary = (batchKey: string) =>
  `/admin/monitoramento/cloudinary/backup/${encodeURIComponent(batchKey)}/download`;
...
const verificarUsoCloudinary = async (options?: { manual?: boolean; forceEmail?: boolean }) => {
  ...
  try {
    ...
    let backupAutomatico:
      | Awaited<ReturnType<typeof executarBackupCloudinary>>
      | null = null;
    ...
    if (manual || acimaDoLimite) {
      console.log(
        `[Cloudinary] uso ${dados.usagePercent.toFixed(2)}% (${formatarBytes(dados.usageBytes)} de ${formatarBytes(dados.limitBytes)})`
      );
    }
    ...
    return {
      ...
      autoBackupBatchKey: backupAutomatico?.batchKey || null,
    ...
    return {
      ok: false,
      enabled: true,
      reason: error?.message || "Erro ao verificar uso do Cloudinary.",
    };
  }
};
...
const iniciarMonitoramentoCloudinary = () => {
  ...
  console.log(
    `[Cloudinary] Monitoramento ativo a cada ${Math.max(CLOUDINARY_MONITOR_INTERVAL_MINUTES, 5)} minuto(s).`
  );
};
...
const ensureImageColumns = async () => {
  ...
  await pool.query("ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS usa_vale_recibo BOOLEAN NOT NULL DEFAULT false");
  await pool.query("ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS nf_imagem_url TEXT");
  await pool.query("ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS vale_recibo_imagem_url TEXT");
  await pool.query("ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS canhoto_imagem_url TEXT");
  ...
  await pool.query(
    `CREATE INDEX IF NOT EXISTS idx_notificacao_dispositivos_usuario
...
app.post("/notificacoes/dispositivos", autenticarToken, async (req: AuthenticatedRequest, res) => {
  const expoPushToken = normalizarExpoPushToken(req.body?.expo_push_token);
  const plataforma = String(req.body?.plataforma || "unknown").trim().slice(0, 30) || "unknown";
    ...
    await pool.query(
      `INSERT INTO notificacao_dispositivos (usuario_id, expo_push_token, plataforma, ativo, atualizado_em)
       ...
       SET usuario_id = EXCLUDED.usuario_id,
           ...
           ativo = true,
           atualizado_em = NOW()`,
      [req.user?.id, expoPushToken, plataforma]
    );
    return res.json({ ok: true });
  } catch (error) {
...
app.delete("/notificacoes/dispositivos", autenticarToken, async (req: AuthenticatedRequest, res) => {
  const expoPushToken = normalizarExpoPushToken(req.body?.expo_push_token);
  if (!expoPushToken) {
    return res.status(400).json({ error: "expo_push_token inválido." });
  }
...
app.get(["/compartilhar/pedido/:id/nf", "/nf/:id", "/n/:id"], async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT nf_imagem_url, vale_recibo_imagem_url
       FROM pedidos
       WHERE id = $1`,
      [pedidoId]
    );
...
app.post("/admin/monitoramento/cloudinary/verificar", async (req: AuthenticatedRequest, res) => {
  ...
  const resultado = await verificarUsoCloudinary({
    manual: true,
    forceEmail: normalizarBoolean(req.body?.force_email),
  });
...
app.get("/admin/monitoramento/cloudinary/backup/:batchKey/download", async (req: AuthenticatedRequest, res) => {
  if (req.user?.perfil !== "admin" && req.user?.perfil !== "backoffice") {
...
app.patch("/pedidos/nf/antecipar", async (req: AuthenticatedRequest, res) => {
  const pedidoIdsRaw: unknown[] | null = Array.isArray(req.body?.pedido_ids) ? req.body.pedido_ids : null;
  if (!pedidoIdsRaw || pedidoIdsRaw.length === 0) {
    return res.status(400).json({ error: "pedido_ids é obrigatório e deve ter ao menos 1 item." });
  }
...
app.post("/pedidos", async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const clienteResult = await client.query(
      "SELECT codigo_cliente, nome FROM clientes WHERE id = $1",
      [cliente_id]
    );
    ...
    const pedidoResult = await client.query(
      `INSERT INTO pedidos (chave_pedido, cliente_id, rota_id, data, status, valor_total, usa_nf, usa_vale_recibo, nf_imagem_url, vale_recibo_imagem_url, canhoto_imagem_url, nf_numero, nf_status, criado_por)
...
app.delete("/pedidos/:id", requireRoles("admin"), async (req: AuthenticatedRequest, res) => {
  const { id } = req.params;
